...
router.get('/game/:gameId', async (req: Request, res: Response) => {
  try {
    ...
    const game = await queryOne(`SELECT id FROM clubhouse_games WHERE id = $1`, [gameId]);
    if (!game) {
...
router.put('/game/:gameId', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { gameId } = req.params;
    ...
    try {
      ...
      const existingMeal = await queryOne<Meal>(`
        SELECT * FROM clubhouse_meals WHERE game_id = $1
      `, [gameId]);
...
router.post('/', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const game = await queryOne(`SELECT id FROM clubhouse_games WHERE id = $1`, [game_id]);
    if (!game) {
      return res.status(404).json({ error: 'Game not found' });
    }
...
router.delete('/:id', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const rowCount = await execute(`DELETE FROM clubhouse_meals WHERE id = $1`, [id]);
