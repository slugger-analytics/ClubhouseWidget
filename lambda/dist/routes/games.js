...
router.get('/', async (req, res) => {
    try {
        const games = await (0, pool_1.query)(`
      SELECT g.*,
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const game = await (0, pool_1.queryOne)(`
      SELECT g.*,
             ht.team_name as home_team_name,
             at.team_name as away_team_name
      FROM clubhouse_games g
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const homeTeam = await (0, pool_1.queryOne)(`SELECT id FROM clubhouse_teams WHERE id = $1`, [home_team_id]);
        if (!homeTeam) {
            return res.status(404).json({ error: 'Home team not found' });
        }
        const awayTeam = await (0, pool_1.queryOne)(`SELECT id FROM clubhouse_teams WHERE id = $1`, [away_team_id]);
        if (!awayTeam) {
...
router.put('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (home_team_id) {
            const team = await (0, pool_1.queryOne)(`SELECT id FROM clubhouse_teams WHERE id = $1`, [home_team_id]);
            if (!team) {
                return res.status(404).json({ error: 'Home team not found' });
            }
        }
        if (away_team_id) {
            const team = await (0, pool_1.queryOne)(`SELECT id FROM clubhouse_teams WHERE id = $1`, [away_team_id]);
            if (!team) {
                return res.status(404).json({ error: 'Away team not found' });
            }
        }
...
router.delete('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const rowCount = await (0, pool_1.execute)(`DELETE FROM clubhouse_games WHERE id = $1`, [id]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Game not found' });
        }
        res.status(204).send();
