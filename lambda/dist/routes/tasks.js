...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        // Verify user exists
        const user = await (0, pool_1.queryOne)(`SELECT id FROM clubhouse_users WHERE id = $1`, [userId]);
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        const tasks = await (0, pool_1.query)(`
...
router.get('/user/:userId/status/:complete', async (req, res) => {
    try {
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const task = await (0, pool_1.queryOne)(`
...
router.post('/me', auth_1.authMiddleware, async (req, res) => {
    try {
        const sluggerUserId = req.user?.cognitoSub;
        if (!sluggerUserId) {
            return res.status(401).json({ error: 'User not authenticated' });
    ...
    `, [sluggerUserId]);
        ...
        const { task_name, task_description, task_complete, task_category, task_type, task_date, task_time, is_repeating, repeating_day } = req.body;
        const task = await (0, pool_1.queryOne)(`
...
router.delete('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
      ...
      `, [sluggerUserId]);
        ...
        const rowCount = await (0, pool_1.execute)(`DELETE FROM clubhouse_tasks WHERE id = $1`, [id]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Task not found' });
        }
        res.status(204).send();
